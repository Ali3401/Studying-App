/* Lucid — the prompt you give Claude so its output drops straight into the app. */

export const PROMPT = `You are writing study notes that will be pasted into "Lucid", a note-reading app.
...
export const SHORT_PROMPT = `Write this as study notes in Lucid Markdown: front matter (title, subject, tags, emoji, accent), \`##\`/\`###\` sections, short paragraphs, and these blocks where they earn their place — \`::: key\`, \`::: definition\`, \`::: formula\`, \`::: tip\`, \`::: warn\`, \`::: exam\`, \`::: example\`, \`::: mnemonic\`, \`::: steps\`, \`::: compare A vs B\` (columns split by \`|||\`), \`::: summary\` at the end, then a \`::: quiz\` block of 5–10 \`Q:\`/\`A:\` pairs. Close every block with \`:::\`. Output only the note.`;

export const SYNTAX = [
  { code: '## Section\n### Sub-section', desc: 'Headings. They build the outline sidebar.' },
  { code: '**bold**  *italic*  `code`', desc: 'Inline emphasis.' },
  { code: '==important==', desc: 'Marks text as important while you write.' },
  { code: '~~struck~~   ^sup^   ~sub~', desc: 'Strikethrough, superscript, subscript.' },
  { code: '- bullet\n  - nested bullet\n1. numbered', desc: 'Lists, nested by indentation.' },
  { code: '- [ ] to do\n- [x] done', desc: 'Checklists you can tick while reading.' },
  { code: '> a quotation', desc: 'Block quote.' },
  { code: '| Drug | Dose |\n| --- | --- |\n| Aspirin | 300mg |', desc: 'Tables.' },
  { code: 'Preload :: the stretch before contraction', desc: 'Definition list row.' },
  { code: '$$\nCO = HR × SV\n$$', desc: 'Display formula.' },
  { code: '::: key Title\nbody\n:::', desc: 'Callout. Types: key, note, tip, warn, exam, example, definition, formula, summary, mnemonic.' },
  { code: '::: steps\n- first\n- second\n:::', desc: 'Numbered step sequence.' },
  { code: '::: compare A vs B\nleft\n|||\nright\n:::', desc: 'Two columns.' },
  { code: '::: quiz\nQ: question\nA: answer\n:::', desc: 'Collapsible self-test — also becomes flashcards.' },
  { code: '![caption](img:…)', desc: 'An image you pasted or dropped in.' },
  { code: '---', desc: 'A divider.' },
];
